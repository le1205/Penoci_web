...
const COM_NAME = ['secudata', 'medi', 'livefurniture', 'virtualbuy', 'ReadSimpl', 'PingNTouch'];
...
export const _brandsColor = BRANDS_NAME.map((brand, index) => ({
  id: _mock.id(index),
  name: brand,
  image: `/assets/logo/logos/${brand}.png`,
}));

export const _comeName = COM_NAME.map((com, index) => ({
  id: _mock.id(index),
  name: com,
  image: `/assets/logo/logo-${com}.svg`,
}));
